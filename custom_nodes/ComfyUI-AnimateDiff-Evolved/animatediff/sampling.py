...
def evolved_sampling_function(model, x, timestep, uncond, cond, cond_scale, model_options: dict={}, seed=None):
    ...
    if not ADGS.is_using_sliding_context():
        ...
        cond_pred, uncond_pred = sliding_calc_cond_uncond_batch(model, cond, uncond_, x, timestep, model_options, cond_scale)

    def get_cfg_result():
        if "sampler_cfg_function" in model_options:
            print("****** Using sampler_cfg_function")
        ...
        for fn in model_options.get("sampler_post_cfg_function", []):
            print(f"******Using post cfg function: {fn.__name__}")
    ...
    return get_cfg_result()


# sliding_calc_cond_uncond_batch inspired by ashen's initial hack for 16-frame sliding context:
# https://github.com/comfyanonymous/ComfyUI/compare/master...ashen-sensored:ComfyUI:master
def sliding_calc_cond_uncond_batch(model, cond, uncond, x_in: Tensor, timestep, model_options, cond_scale: float):
